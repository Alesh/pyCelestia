...
class Client(_jsonrpc.Client):
    ...
    def __init__(self, url: str = None, /, *, auth_token: str = None,
        ...
        self.errors_map = dict((rpc_err, ValueError) for rpc_err in [
            'unmarshaling params',
            'equal to 0',
    ...
    def connect(self, auth_token: str = None, /, response_timeout: float = 180,
                **_) -> AbstractAsyncContextManager[NodeAPI]:
        ...
        async def connect_context():
            async with super(Client, self).connect(additional_headers=headers,
                                                   errors_map=self.errors_map,
                                                   response_timeout=response_timeout,
                                                   json_encoder=JSONEncoder) as rpc:
                yield NodeAPI(rpc)
