...
def test_create_blob():
    ...
    signed_blob = Blob(b'abc', b'0123456789', signer='celestia1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5wgawu3')
...
async def test_sending_signed_blob(node_provider):
    ...
    async with client.connect(auth_token) as api:
        ...
        # bad signer
        bad_signer_blob = Blob(b'abc', b'0123456789', signer='celestia1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5wgawu3')
        ...
        with pytest.raises(ValueError, match="`signer_address` does not match the current account address"):
            await api.blob.submit(bad_signer_blob, signer_address='celestia1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5wgawu3')

        # bad BLOB signer auto
        bad_signer_blob = Blob(b'abc', b'0123456789',
                               signer='celestia1qypqxpq9qcrsszg2pvxq6rs0zqg3yyc5wgawu3')
        with pytest.raises(ValueError, match="`signer_address` of blob does not match the current account address"):
...
async def test_blob_subscribe(node_provider):
    ...
    assert tuple(ext.bytes2address(item.blobs[0].signer) for item in result) == (account_address,) * 4
